...
void Modbus::begin(long u32speed)
{
    ...
    if (u8txenpin > 1)   // pin 0 & pin 1 are reserved for RX/TX
    {
        ...
        GPIO_PinMode(u8txenpin, OUTPUT);
        GPIO_DigitalWrite(u8txenpin, LOW);
    }
...
{
    return ((unsigned long)(RTC_millis() -u32timeOut) > (unsigned long)u16timeOut);
}
...
int8_t Modbus::poll()
{
    ...
    if ((unsigned long)(RTC_millis() -u32timeOut) > (unsigned long)u16timeOut)
    {
        u8state = COM_IDLE;
    ...
    if ((unsigned long)(RTC_millis() -u32time) < (unsigned long)T35) return 0;
...
int8_t Modbus::poll( uint16_t *regs, uint8_t u8size )
{
    ...
    if (u8current != u8lastRec)
    {
        printf("current: %d lastRec: %d\n", u8current, u8lastRec);
        u8lastRec = u8current;
        u32time = RTC_millis();
        printf("Frame is not end\n");
        return 0;
    }
    printf("Frame is end; current: %d lastRec: %d\n", u8current, u8lastRec);
    if ((unsigned long)(RTC_millis() -u32time) < (unsigned long)T35) return 0;

    u8lastRec = 0;
    int8_t i8state = getRxBuffer();
    printf("getRxBuffer return value %d\n", i8state);
    u8lastError = i8state;
    if (i8state < 7) return i8state;

    // check slave id
    if (au8Buffer[ ID ] != u8id) {
        printf("id is not ok %d\n", au8Buffer[ ID ]);
        return 0;
    }
    printf("check id is ok\n");

    // validate message: CRC, FCT, address and size
    uint8_t u8exception = validateRequest();
    if (u8exception > 0)
    {
        printf("Validate exception failed %d\n", u8exception);
...
int8_t Modbus::getRxBuffer()
{
    ...
    if (u8txenpin > 1) GPIO_DigitalWrite( u8txenpin, LOW );
...
{
    printf("process write multiple registers\n");
